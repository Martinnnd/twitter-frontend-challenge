...
const TweetBox = ({ parentId, close, mobile }: TweetBoxProps) => {
  ...
  const createCommentMutation = useMutation({
    ...
    mutationFn: ({
      ...
      images: string[];
      parentId: string;
    }) => service.commentPost(parentId, content, images),
    onSuccess: (data) => {
  ...
  const createPostMutation = useMutation({
    ...
    onSuccess: () => {
        ...
        queryClient.refetchQueries({ queryKey: ["posts"] });  // 💡 Fuerza una actualización
    }
});
  ...
  const handleSubmitImages = async (files: File[]): Promise<string[]> => {
    ...
    for (const file of files) {
      console.log("Solicitando presigned URL para:", file.name);
      const res = await service.addImage(file.type);

      if (!res || !res.presignedUrl || !res.fileUrl) {
        console.error("❌ Error: Respuesta inválida de addImage", res);
        throw new Error("No se pudo obtener una URL pre-firmada");
      }
