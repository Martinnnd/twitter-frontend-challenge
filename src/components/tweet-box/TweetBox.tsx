...
const TweetBox = ({ parentId, close, mobile }: TweetBoxProps) => {
  ...
  const createPostMutation = useMutation({
    ...
    mutationFn: ({ content, images }: { content: string; images: string[] }) =>
      service.createPost({ content, images }),
    onSuccess: () => {
      queryClient.invalidateQueries({
        queryKey: ["infinitePosts"],
      });
  ...
  const handleSubmitImages = async (files: File[]): Promise<string[]> => {
    ...
    for (const file of files) {
      ...
      if (!res || !res.presignedUrl) {
        throw new Error("No se recibió una URL válida para subir la imagen.");
      }
  ...
  const handleSubmit = async () => {
    try {
      ...
      addToast({
        message: t("Tweet uploaded successfully"),
        type: ToastType.SUCCESS,
        show: true,
      });
      close && close();
    } catch (e) {
      addToast({ message: t("error "), type: ToastType.ALERT, show: true });
    }
  };
